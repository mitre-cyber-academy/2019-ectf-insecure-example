...
* Sometimes the kernel may crash during boot. This appears to only happen when a monitor is connected, but this has not been confirmed. The best 
  guess currently is that this is being caused by a race condition in the DRM driver (and possibly the Digilent DRM encoder). Current work-around
  is to either not attach a monitor, or reset the board until boot completes successfully. Another work-around may be to attach a monitor after 
  boot has completed, but this has not been verified. 
...
```
git clone --recursive https://github.com/Digilent/Petalinux-Arty-Z7-10.git
```
If you are not planning on version controlling your project and want a simpler release package, go to https://github.com/Digilent/Petalinux-Arty-Z7-10/releases/
